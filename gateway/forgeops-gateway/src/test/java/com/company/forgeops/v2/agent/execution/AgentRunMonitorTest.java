...
class AgentRunMonitorTest {
    ...
    @Test
    void aValidButUnverifiedCodingPrDeclarationCannotAdvanceTheWorkflow() {
        ...
        new AgentRunMonitor(runs, execution, workflow, JsonMapper.shared()).inspectRunning();

        verify(workflow).recordRuntimeFailure(eq(run.getId()), eq(AgentRunState.FAILED), eq("DELIVERY_EVIDENCE_REQUIRED"),
                startsWith("monitor:"));
    }
}
